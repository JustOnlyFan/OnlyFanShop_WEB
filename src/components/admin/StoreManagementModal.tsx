...
export default function StoreManagementModal({ store, onClose, onSaved }: StoreManagementModalProps) {
  ...
  const handleGeocodeFromAddress = async () => {
    const query = `${formData.address}, ${formData.ward || ''}, ${formData.district || ''}, ${formData.city || ''}`.trim()
    if (!query) {
    ...
    try {
      ...
      if (Array.isArray(results) && results.length > 0) {
        ...
        if (Number.isFinite(latitude) && Number.isFinite(longitude)) {
          setFormData(prev => ({ ...prev, latitude, longitude }))
          toast.success('Đã lấy toạ độ từ địa chỉ')
        } else {
              ...
              <div className="md:col-span-2">
                <div className="flex items-center justify-between">
                  ...
                  <button
                    ...
                    className="px-4 py-2 rounded-full inline-flex items-center gap-2 text-sm shadow-sm transition-colors border bg-white text-gray-700 border-gray-300 hover:bg-gray-50"
                    title="Mở popup chọn địa chỉ trên bản đồ"
                  >
                    ...
                    {(formData.images || []).map((url, idx) => (
                      <div key={`${url}-${idx}`} className="relative group">
                        ...
                        <button
                          ...
                          className="absolute top-1 right-1 p-1 bg-white/80 rounded-full border text-red-600 opacity-0 group-hover:opacity-100 transition"
                          title="Xoá ảnh"
                        >
          ...
          {addressModalOpen && (
            <AddressPickerModal
              initial={{
                ...
                ward: formData.ward || '',
                latitude: formData.latitude || 10.776,
                longitude: formData.longitude || 106.7
              }}
