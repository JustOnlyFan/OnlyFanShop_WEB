...
export default function NewInventoryRequestPage() {
  ...
  const loadData = async () => {
    try {
      ...
      // Load products enabled for this store
      const productsResp = await StoreInventoryService.getStoreProducts(storeId, true)
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      router.push(`/admin/stores/${storeId}/inventory-requests`)
    } catch (e: any) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500">Chọn sản phẩm và số lượng cần nhập cho {store?.name}</p>
        </div>
      </div>
