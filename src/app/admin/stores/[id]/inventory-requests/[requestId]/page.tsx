...
export default function InventoryRequestDetailPage() {
  ...
  }, [hasHydrated, isAuthenticated, user?.role, storeId, requestId])
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-2 text-sm">
        ...
        <span className="text-gray-400">/</span>
        <Link href={`/admin/stores/${storeId}/inventory-requests`} className="text-gray-500 hover:text-indigo-600 transition-colors">
          Yêu cầu nhập hàng
        </Link>
        <span className="text-gray-400">/</span>
        ...
        <div className="lg:col-span-2 space-y-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-4 mt-1 text-sm">
                      <span className="text-gray-500">
                        Yêu cầu: <span className="font-semibold text-gray-900">{item.requestedQuantity}</span>
                      </span>
                      {item.approvedQuantity !== undefined && item.approvedQuantity !== null && (
                        <span className="text-green-600">
                          Duyệt: <span className="font-semibold">{item.approvedQuantity}</span>
                        </span>
                      )}
                    </div>
                  </div>
          ...
          {(request.requestNote || request.adminNote) && (
            <div className="bg-white rounded-2xl border border-gray-200 p-4 space-y-3">
              {request.requestNote && (
                <div>
                  <p className="text-sm font-medium text-gray-500 mb-1">Ghi chú từ cửa hàng</p>
                  <p className="text-gray-900">{request.requestNote}</p>
                </div>
              )}
              {request.adminNote && (
                <div>
                  <p className="text-sm font-medium text-gray-500 mb-1">Ghi chú từ admin</p>
                  <p className="text-gray-900">{request.adminNote}</p>
                </div>
              )}
            </div>
          )}
          ...
          <div className="bg-white rounded-2xl border border-gray-200 p-4 space-y-4">
            ...
            <div className="space-y-3 text-sm">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="font-medium text-gray-900">{request.requesterName || 'N/A'}</p>
                </div>
              </div>
              ...
              {request.approverName && (
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="font-medium text-gray-900">{request.approverName}</p>
                  </div>
                </div>
              )}
              ...
              {request.approvedAt && (
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="font-medium text-gray-900">{new Date(request.approvedAt).toLocaleString('vi-VN')}</p>
                  </div>
                </div>
              )}
              ...
              {request.completedAt && (
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="font-medium text-gray-900">{new Date(request.completedAt).toLocaleString('vi-VN')}</p>
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="pt-3 border-t border-gray-100">
              <div className="flex justify-between text-sm">
                ...
                <span className="font-semibold text-gray-900">{items.length} loại</span>
              </div>
              <div className="flex justify-between text-sm mt-1">
                <span className="text-gray-500">Tổng số lượng</span>
                <span className="font-semibold text-gray-900">{items.reduce((sum, i) => sum + i.requestedQuantity, 0)}</span>
              </div>
            </div>
          </div>
