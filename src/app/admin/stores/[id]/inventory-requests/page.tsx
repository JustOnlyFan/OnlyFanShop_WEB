...
export default function InventoryRequestsPage() {
  ...
  const loadData = async () => {
    try {
      ...
      try {
        const resp = await InventoryRequestService.getStoreRequests(storeId)
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">Quản lý đơn yêu cầu nhập số lượng sản phẩm cho {store?.name}</p>
          </div>
        </div>
        <Link
          href={`/admin/stores/${storeId}/inventory-requests/new`}
          ...
          {Object.entries(statusConfig).map(([key, config]) => {
            ...
            return (
              <button
                ...
                className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
                  statusFilter === key ? 'bg-gray-900 text-white' : `${config.color} hover:opacity-80`
                }`}
              >
                {config.label} ({count})
              </button>
            ...
            {filteredRequests.map((request, idx) => {
              ...
              return (
                <motion.div
                  ...
                  onClick={() => router.push(`/admin/stores/${storeId}/inventory-requests/${request.id}`)}
                >
                  <div className="flex items-center justify-between gap-4">
                    ...
                    <div className="flex items-center gap-3" onClick={(e) => e.stopPropagation()}>
                      ...
                      {request.status === 'APPROVED' && (
                        <button
                          onClick={() => handleStartShipping(request.id)}
                      ...
                      <Link
                        href={`/admin/stores/${storeId}/inventory-requests/${request.id}`}
