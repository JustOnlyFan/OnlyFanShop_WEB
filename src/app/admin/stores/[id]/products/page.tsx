...
export default function StoreProductsPage() {
  ...
  const loadData = async () => {
    try {
      ...
      const productsResp = await StoreInventoryService.getAllProductsWithStoreStatus(storeId)
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-2 text-sm">
        ...
        <span className="text-gray-400">/</span>
        <span className="text-gray-900 font-medium">Sản phẩm cửa hàng</span>
      </div>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Sản phẩm cửa hàng</h1>
            <p className="text-sm text-gray-500">Chọn sản phẩm được phép bán tại {store?.name}</p>
          </div>
        </div>
        <button
          onClick={handleSave}
      ...
      <div className="bg-white rounded-2xl border border-gray-200 p-4">
        ...
        <div className="mt-4 flex items-center gap-6 text-sm">
          ...
          {hasChanges && (
            <span className="text-orange-500 font-medium">• Có thay đổi chưa lưu</span>
          )}
        </div>
      </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {filteredProducts.map((product, idx) => {
              ...
              return (
                ...
                >
                  {/* Checkbox */}
                  <div className={`absolute top-3 right-3 w-6 h-6 rounded-lg border-2 flex items-center justify-center ${
